...
HRESULT CUI_Responcer::Initialize_Prototype()
{
	...
	CUI_Object* pDialogue = Builder::Create_UIObject({ "GamePlay_Level", "GamePlay_GameObject_UI_EventMsg" })
		.Add_To_Level("GamePlay_Level")
		.Position({ 600,620.f })
		.Build("EvtMsg");

	CGameInstance::GetInstance()->Get_UIMgr()->Add_UIObject(pDialogue, "GamePlay_Level");
	m_UIContainer.emplace("EvtMsg",pDialogue);
	return S_OK;
}
