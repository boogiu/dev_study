#include "pch.h"
#include "CTutorialStage.h"
#include "CResourceMgr.h"
#include "CObject.h"
#include "CPlayer.h"
#include "CObjMgr.h"
#include "CCamera.h"
#include "CInfiniteObj.h"
#include "CScene.h"
#include "CBattleScene.h"
#include "Key_Item.h"

CTutorialStage::CTutorialStage()
{
}

CTutorialStage::~CTutorialStage()
{
	Release();
}

void CTutorialStage::Initialize()
{
	__super::Load_Map(L"../TileData/TutorMap.dat");

	CResourceMgr::Get_Instance()->PlayBGM(L"Back_Ground", 0.1f);

	CCamera::Get_Instance()->Set_Border(MAP_CX, MAP_CY);
	CObject* player = CObjMgr::Get_Instance()->ReQuestObject<CPlayer>(PLAYER);
	player->Set_MoveSCLimit(false);

	for (int i = 0; i < 6; ++i)
	{
		int width = 340;
		CInfiniteObj* tmp = new CInfiniteObj;
		tmp->Initialize();
		INFO_EX myInfo;

		myInfo.fX = i * width;
		myInfo.fY = WINCY >> 1;
		myInfo.Add_OffsetX(width * 0.5f, width * 0.5f);
		myInfo.Add_OffsetY((WINCY >> 1) + 50, (WINCY >> 1) + 50);
		tmp->Set_Info(myInfo);
		tmp->Set_SpriteScale(1920, 400);
		tmp->Set_Index(i);
		tmp->LoadSprite(L"BackWall");
		tmp->Set_MoveSpeed(0.3f);

		m_BackStage.push_back(tmp);
	}
	for (int i = 0; i < 5; ++i)
	{
		int width = 80;
		CInfiniteObj* tmp = new CInfiniteObj;
		tmp->Initialize();
		INFO_EX myInfo;
		myInfo.fX = i * width;
		myInfo.fY = 0;
		myInfo.Add_OffsetX(width * 0.5f, width * 0.5f);
		myInfo.Add_OffsetY(0, WINCY / 3);
		tmp->Set_Info(myInfo);
		tmp->Set_SpriteScale(1600, 578);
		tmp->Set_Index(i);
		tmp->LoadSprite(L"BackCutton");
		tmp->Set_MoveSpeed(1.2f);
		m_BackCutton.push_back(tmp);
	}
	for (int i = 0; i < 6; ++i)
	{
		int width = 150;
		CInfiniteObj* tmp = new CInfiniteObj;
		tmp->Initialize();
		INFO_EX myInfo;
		myInfo.fX = i * width;
		myInfo.fY = WINCY + 0;
		myInfo.Add_OffsetX(width * 0.5f, width * 0.5f);
		myInfo.Add_OffsetY(WINCY / 2, 0);
		tmp->Set_Info(myInfo);
		tmp->Set_SpriteScale(1920, 383);
		tmp->Set_Index(i);
		tmp->LoadSprite(L"BackBridge");
		tmp->Set_MoveSpeed(0.6f);
		m_BackBridge.push_back(tmp);
	}

	__super::Set_Player_Pos();

	Key_Item* Key = static_cast<Key_Item*>(CObjMgr::Get_Instance()->ReQuestObject<Key_Item>(ITEM));
	Key->Initialize();
	Key->Set_Name(L"Z_Key");
	Key->Set_Pos(850, 870);
	Key->Set_Origin(870);

	Key = static_cast<Key_Item*>(CObjMgr::Get_Instance()->ReQuestObject<Key_Item>(ITEM));
	Key->Initialize();
	Key->Set_Name(L"X_Key");
	Key->Set_Pos(1050, 870);
	Key->Set_Origin(870);

	Key = static_cast<Key_Item*>(CObjMgr::Get_Instance()->ReQuestObject<Key_Item>(ITEM));
	Key->Initialize();
	Key->Set_Name(L"C_Key");
	Key->Set_Pos(1250, 870);
	Key->Set_Origin(870);

	Key = static_cast<Key_Item*>(CObjMgr::Get_Instance()->ReQuestObject<Key_Item>(ITEM));
	Key->Initialize();
	Key->Set_Name(L"A_Key");
	Key->Set_Pos(1450, 870);
	Key->Set_Origin(870);

	Key = static_cast<Key_Item*>(CObjMgr::Get_Instance()->ReQuestObject<Key_Item>(ITEM));
	Key->Initialize();
	Key->Set_Name(L"S_Key");
	Key->Set_Pos(1650, 870);
	Key->Set_Origin(870);

	CObject* Orge = CObjMgr::Get_Instance()->ReQuestObject<CNpc>(MONSTER);
	Orge->Initialize();
	Orge->Set_Pos(2150, 870);

	Key = static_cast<Key_Item*>(CObjMgr::Get_Instance()->ReQuestObject<Key_Item>(ITEM));
	Key->Initialize();
	Key->Set_Name(L"SPACE_Key");
	Key->Set_Pos(2550, 870);
	Key->Set_Origin(870);
	Key->Get_Info().Add_OffsetX(20.f, 20.f);


}

void CTutorialStage::Update()
{
	for (int i = 0; i < (int)TILE_END;++i) {
		for (CObject* tile : m_StageTile[(TILE_TYPE)i]) {
			if (tile->Is_Activate())
				tile->Update();
		}
	}

	if (CObjMgr::Get_Instance()->ReQuestObject<CPlayer>(PLAYER)->Get_Info().fX >= 4495) {
		m_bStageEnd = true;
	}
	
	Update_BackObj();
}

void CTutorialStage::Late_Update()
{
	if (m_bStageEnd) {
		static_cast<CStageDoor*>(m_StageDoor)->Open_Door();
	}
	if (static_cast<CStageDoor*>(m_StageDoor)->Next_Stage()) {
		static_cast<CBattleScene*>(m_pOwnScene)->Change_Stage();
	}
}

void CTutorialStage::Render(HDC _hDC)
{
	memDC = CResourceMgr::Get_Instance()->Find_Image(L"Ground");
	BitBlt(_hDC, 0, 0, WINCX, WINCY, memDC, 150, 0, SRCCOPY);

	Render_BackObj(_hDC);

	for (int i = 0; i < (int)TILE_END;++i) {
		for (CObject* tile : m_StageTile[(TILE_TYPE)i]) {
			if (CCamera::Get_Instance()->Out_of_Camera(tile->Get_Info())) {
				continue;
			}
			if (tile->Is_Activate())
				tile->Render(_hDC);
		}
	}
	
}

void CTutorialStage::Release()
{
	for (CInfiniteObj*& obj : m_BackStage) {
		Safe_Delete<CInfiniteObj*>(obj);
	}
	for (CInfiniteObj*& obj : m_BackCutton) {
		Safe_Delete<CInfiniteObj*>(obj);
	}
	for (CInfiniteObj*& obj : m_BackBridge) {
		Safe_Delete<CInfiniteObj*>(obj);
	}
}
